...
class WNet(nn.Module):
    ...
    def __init__(self, args, masked_kspace=True):
        ...
        # For FastMRI dataset, mask_path is None and masks are embedded in data
        if mask_path is None:
            # Create a default mask for FastMRI - this will be overridden by actual masks from data
            self.mask = torch.ones((args.img_size, args.img_size), device=self.args.device)
        elif args.mask_type=='radial' and args.sampling_percentage==30:
    ...
    def inverseFT(self, Kspace):
        ...
        # Convert to complex tensor for new PyTorch FFT API
        Kspace_complex = torch.complex(Kspace[..., 0], Kspace[..., 1])
        img_cmplx = torch.fft.ifft2(Kspace_complex)
        img = torch.abs(img_cmplx)
        img = img[:, None, :, :]
        return img
